"""EEG Signal Processing for PhysioNet Dataset."""
    ...
    def extract_trials(self, raw, run=None):
        ...
        if len(events) == 0:
            ...
            if n_samples >= self.window_samples:
                ...
                return np.empty((0, data.shape[0], self.window_samples), dtype=np.float32), np.empty((0,), dtype=np.int64)
        else:
            epochs = mne.Epochs(
                raw, events, event_id,
...
def process_all_files(data_dir="./data/raw", num_subjects=109):
    """Process subjects from PhysioNet dataset and save to disk."""
    processor = EEGSignalProcessor()
    all_X = []
    all_y = []

    print(f"\n{'='*60}")
    print(f"PROCESSING ALL {num_subjects} SUBJECTS")
    print(f"{'='*60}")
        ...
        for run in range(3, 15):
            ...
            if os.path.exists(filepath):
                ...
                if X is not None and len(X) > 0:
                    all_X.append(X)
                    all_y.append(y)
                    subject_trials += len(X)

        if subject_trials > 0:
            print(f"  {subject_id}: {subject_trials} trials")
    ...
    print(f"Data mean, std:     {X.mean():.4f}, {X.std():.4f}")
...
if __name__ == "__main__":
    process_all_files("./data/raw", num_subjects=5)
